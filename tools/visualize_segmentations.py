...
"""
visualize_segmentations.py - Draw the lane segmentations for each location.

This script iterates over the orthophoto folder and draws the lane segmentation for each orthophoto based on matching CSV files in the segmentation folder.

Usage:
  python tools/visualize_segmentations.py --ortho-folder <ortho_folder> --segmentation-path <segmentation_path> --ortho-suffix <suffix>

Arguments:
  --ortho-folder : Path to the folder containing orthophoto files (.png, .tif, .tiff).
  --segmentation-path : Path to the folder containing lane and road segmentation csv files.
  --ortho-suffix : Suffix of the orthophoto files (e.g., 'png', 'tif').
"""

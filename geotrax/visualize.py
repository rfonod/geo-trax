...
the stabilized frame (1), or the static reference frame (2). Modes 3 and 4 instead draw an oriented best-fit box
per vehicle — sized to its best-estimated length/width and rotated to its per-frame heading (computed from the
...
Visualization Options:
  ...
  --viz-mode, -vm <int> [<int> ...] : Frame source(s) for annotation: 0=original, 1=stabilized,
                        ...
                        4=oriented best-fit boxes on the stabilized frame (3 and 4 rotate each
                        box to the vehicle's per-frame heading; both require stabilization).
    ...
    for mode in modes:
        if mode not in (0, 1, 2, 3, 4):
            logger.critical(
                ...
                f"2 (reference), 3 (oriented best-fit boxes on the original frame), "
                f"4 (oriented best-fit boxes on the stabilized frame)."
            )
            sys.exit(1)
...
def read_tracks_oriented(tracks: pd.DataFrame, tracks_txt_filepath: Path, class_names: dict, args: argparse.Namespace, logger: logging.Logger) -> tuple:
    """
    Build the slim per-row layout for the oriented best-fit box modes (3 and 4).
...
def annotate_frame(frame: np.ndarray, frame_num: int, tracks_frame: pd.DataFrame, track_history: dict, class_names: dict, speed_lane_frame: pd.DataFrame, viz_config: dict, args: argparse.Namespace, logger: logging.Logger, Hinv: np.ndarray = None) -> np.ndarray:
    ...
    (cols 2/3 = stabilized center, 4/5 = best-fit length/width, 8 = heading) and ``Hinv`` projects
    each box from stabilized space onto the rendered frame: the inverse stabilization homography
    for mode 3 (original frame), or the identity for mode 4 (already-stabilized frame).
    """
...
def draw_oriented_box(frame: np.ndarray, cx: float, cy: float, length: float, width: float, heading: float, Hinv: np.ndarray, color: tuple, line_width: int) -> tuple:
    """
    Draw an oriented best-fit box (visualization modes 3 and 4) and return its projected center.

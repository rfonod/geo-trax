...
Four rendering modes are available (see --viz-mode): the annotations can be drawn on the original frame (0),
the stabilized frame (1), or the static reference frame (2). Mode 3 instead draws an oriented best-fit box per
vehicle — sized to its best-estimated length/width and rotated to its per-frame heading (computed from the
camera-motion-free stabilized trajectory and projected back onto the original frame); it requires stabilization.
...
Visualization Options:
  ...
  --viz-mode, -vm <int> [<int> ...] : Frame source(s) for annotation: 0=original, 1=stabilized,
                        ...
                        (rotated to each vehicle's per-frame heading; requires stabilization).
                        Accepts multiple values (e.g. 0 1 3) to render one video per mode.
  ...
  --heading-smoothing, -hsm <int> : (mode 3) Gaussian smoothing window [frames] for the per-frame heading; larger = steadier, slower to follow turns. Defaults to cfg -> visualization -> heading_smoothing.
  --heading-min-speed, -hms <float> : (mode 3) Minimum smoothed pixel speed [px/frame] for a reliable heading; below this the heading is held from the last reliable frame. Defaults to cfg -> visualization -> heading_min_speed.
...
7. Render oriented best-fit boxes rotated to each vehicle's heading (requires stabilization):
   geotrax visualize path/to/video.mp4 --save --viz-mode 3
...
- For --viz-mode 1 and 3, transformation matrices are read from '<stem><stab_transform_postfix>.txt' in the same folder.
    ...
    for mode in modes:
        if mode not in (0, 1, 2, 3):
            logger.critical(
                ...
                f"2 (reference), 3 (oriented best-fit boxes)."
            )
            sys.exit(1)
...
def read_tracks_oriented(tracks: pd.DataFrame, tracks_txt_filepath: Path, class_names: dict, args: argparse.Namespace, logger: logging.Logger) -> tuple:
    """
    Build the slim per-row layout for visualization mode 3 (oriented best-fit boxes).

    Requires the full stabilized + dimension-estimate layout (14 columns). Emits columns
    ``[frame, id, stab_xc, stab_yc, length, width, class, conf, heading]`` where ``length`` /
    ``width`` are the best-fit estimates (falling back to the per-row raw bbox extents where the
    estimate is NaN) and ``heading`` is the per-frame smoothed travel direction (radians, image
    coordinates) from :func:`compute_headings`.
    """
    ...
    # Dimension fallback: where the best-fit estimate is NaN, use the per-row raw bbox extents.
    length = tracks[12].where(tracks[12].notna(), tracks[[4, 5]].max(axis=1))
    width = tracks[13].where(tracks[13].notna(), tracks[[4, 5]].min(axis=1))
...
def compute_headings(tracks: pd.DataFrame, smoothing: float, min_speed: float, logger: logging.Logger) -> pd.Series:
    """
    Per-frame vehicle heading (radians, image coordinates, y-down) for visualization mode 3.
...
def annotate_frame(frame: np.ndarray, frame_num: int, tracks_frame: pd.DataFrame, track_history: dict, class_names: dict, speed_lane_frame: pd.DataFrame, viz_config: dict, args: argparse.Namespace, logger: logging.Logger, Hinv: np.ndarray = None) -> np.ndarray:
    ...
    For visualization mode 3, ``tracks_frame`` carries the slim oriented layout
    (cols 2/3 = stabilized center, 4/5 = best-fit length/width, 8 = heading) and ``Hinv`` is the
    inverse stabilization homography used to project each oriented box onto the original frame.
    """
    ...
    for track_id, xcn, ycn, wn, hn, c, s, heading in zip(ids, Xcn, Ycn, Wn, Hn, classes, scores, headings):
        ...
        if is_oriented:
            ...
            x1n, y1n = draw_oriented_box(annotated_frame, xcn, ycn, wn, hn, heading, Hinv, color, line_width)
...
def draw_oriented_box(frame: np.ndarray, cx: float, cy: float, length: float, width: float, heading: float, Hinv: np.ndarray, color: tuple, line_width: int) -> tuple:
    """
    Draw an oriented best-fit box (visualization mode 3) and return its projected center.

    The box is built in stabilized image space (center ``cx, cy``; size ``length`` x ``width``;
    rotated to ``heading`` radians) and projected onto the original frame via the inverse
    stabilization homography ``Hinv``, then drawn as a closed polygon with a short tick from the
    center toward the front edge to indicate travel direction. A NaN ``heading`` (no reliable
    motion) falls back to an axis-aligned box.
    """
...
def add_visualization_args(group, include_frame_range: bool = True) -> None:
    ...
    group.add_argument('--viz-mode', '-vm', type=int, nargs='+', default=None, choices=[0, 1, 2, 3], metavar='MODE',
                       help='Frame source(s) for annotation: 0=original, 1=stabilized, 2=reference frame, 3=oriented best-fit boxes on the original frame (rotated to per-frame heading; requires stabilization). Accepts multiple values, e.g. "--viz-mode 0 1 3" renders one video per mode. Defaults to cfg -> visualization -> viz_mode.')
    ...
    group.add_argument('--heading-smoothing', '-hsm', type=int, default=None,
                       help='(mode 3) Gaussian smoothing window [frames] for the per-frame heading; larger = steadier, slower to follow turns. Defaults to cfg -> visualization -> heading_smoothing.')
    group.add_argument('--heading-min-speed', '-hms', type=float, default=None,
                       help='(mode 3) Minimum smoothed pixel speed [px/frame] for a reliable heading; below this the heading is held from the last reliable frame. Defaults to cfg -> visualization -> heading_min_speed.')
